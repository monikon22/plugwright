 ...
 * bot's own message buffer, kept separate from `session.messages` so console traffic never
 * mixes with — or gets cleared alongside — a test player's chat log.
 *
...
export class AdminBotConsole implements ServerConsole {
    ...
    async probe(): Promise<boolean> {
        ...
        try {
            ...
            bot.on('message', (jsonMsg: unknown) => this.buffer.push(String(jsonMsg)));
    ...
    async executeAndWait(cmd: string, timeoutMs: number = 5000): Promise<string> {
        const since = this.buffer.length;
        this.execute(cmd);

        const deadline = Date.now() + timeoutMs;
        while (Date.now() < deadline) {
            const lines = this.buffer.slice(since);
